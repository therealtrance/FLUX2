...
function ProjectTransferEditor({ project, scenarios, setProjects, setExpandedProjects, setCompareA, setCompareB, setPlanView }) {
  ...
  useEffect(() => {
    const nextTarget = scenarios.find((scenario) => scenario.id !== project.scId)
    setTargetScenarioId(nextTarget?.id || '')
  }, [project.scId, scenarios])
  ...
  const copyProject = () => {
    ...
    const clonedProject = {
      ...project,
      id: uid('p'),
      scId: targetScenarioId,
      name: `${project.name} (${targetScenario?.name || 'Copy'})`,
  ...
  return (
    <div className="roster-editor">
      ...
      <div className="mini-form">
        ...
        <div className="form-actions"><button className="subtab" type="button" onClick={copyProject}>Copy to scenario</button><button className="subtab active" type="button" onClick={moveProject}>Move to scenario</button></div>
      </div>
    </div>
  )
}
...
export default function App() {
  ...
  const duplicateScenario = (scenario) => {
    ...
    const clonedProjects = projects
      .filter((project) => project.scId === scenario.id)
      .map((project) => ({
        ...project,
        id: uid('p'),
        scId: newScenarioId,
        roster: duplicateRoster ? project.roster.map((entry) => ({ ...entry, id: uid('r') })) : [],
      }))
